...
export function CreatePostModal() {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      setStep(1);
      setFormData({ teachingSkill: "", thumbnailUrl: "", wantedSkills: [], description: "" });
    }, 300);
  };
  ...
  return (
    <Modal
      ...
      className="max-w-xl overflow-hidden rounded-2xl p-0 border border-white/20 shadow-2xl glass"
    >
      <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-6 text-white relative overflow-hidden">
        <div className="relative z-10">
          ...
          <h2 className="text-2xl font-black tracking-tight mb-1">Create Skill Post</h2>
          <p className="text-blue-100/80 text-sm">Offer your knowledge. Get what you need.</p>
        </div>
      ...
      <div className="p-8 pt-0 min-h-[280px] relative">
        <AnimatePresence mode="wait" custom={1}>
          {step === 1 && (
            ...
            >
              <div>
                <label className="block text-sm font-bold text-gray-900 mb-2">What are you teaching?</label>
                <Input 
                  ...
                  onChange={(e) => setFormData({...formData, teachingSkill: e.target.value})}
                />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-900 mb-2">Thumbnail URL (Optional)</label>
          ...
          {step === 2 && (
            ...
            >
              <div>
                <label className="block text-sm font-bold text-gray-900 mb-2">What skills do you want in return?</label>
                <div className="flex space-x-2">
                  ...
                  />
                  <Button type="button" onClick={handleAddTag} className="h-12 w-12 shrink-0 bg-gray-900 hover:bg-gray-800 text-white rounded-xl shadow-lg shadow-gray-900/20 transition-all">
                    <Plus size={20} />
                  </Button>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4 p-4 bg-gray-50/80 rounded-xl min-h-[80px] border border-gray-100 shadow-inner">
                  {formData.wantedSkills.length === 0 && (
                    <span className="text-sm text-gray-400 flex items-center w-full justify-center">Add tags above</span>
                  )}
                  {formData.wantedSkills.map((skill) => (
                    ...
                    >
                      {skill}
                      <button type="button" onClick={() => handleRemoveTag(skill)} className="ml-2 hover:bg-blue-200 rounded-full p-0.5 transition-colors">
                        <X size={14} />
                      </button>
                    </motion.span>
                  ))}
        ...
        <div className="flex justify-between mt-8 pt-4">
          <Button variant="ghost" onClick={step === 1 ? closeCreatePost : handleBack} className="text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-xl">
            {step === 1 ? "Cancel" : "Back"}
          </Button>
